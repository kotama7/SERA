...
| `ablation_results` | `list[dict]` | `[]` | 自動アブレーション結果のリスト |
...
### add_ablation_data(data) -> None

自動アブレーション結果を追加するメソッド。

- `data` が `list` の場合: `ablation_results` に全要素を追加（`extend`）
- `data` が `dict` の場合: `ablation_results` に単一要素を追加（`append`）
