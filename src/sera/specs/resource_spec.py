...
class SlurmConfig(BaseModel):
    ...
    sbatch_extra: list[str] = Field(
        default_factory=list, description="Extra sbatch directives"
    )
...
class DockerConfig(BaseModel):
    ...
    image: str = Field(
        "pytorch/pytorch:2.3.0-cuda12.1-cudnn9-devel", description="Docker image"
    )
...
class ApiKeysConfig(BaseModel):
    ...
    semantic_scholar: str = Field(
        "SEMANTIC_SCHOLAR_API_KEY", description="Env var for Semantic Scholar key"
    )
...
class SandboxConfig(BaseModel):
    ...
    experiment_memory_limit_gb: int = Field(
        16, description="Memory limit per experiment in GB"
    )
    isolate_experiments: bool = Field(
        True, description="Run each experiment in an isolated environment"
    )
...
class ResourceSpecModel(BaseModel):
    ...
    compute: ComputeConfig = Field(
        default_factory=ComputeConfig, description="Compute resources"
    )
    ...
    network: NetworkConfig = Field(
        default_factory=NetworkConfig, description="Network policy"
    )
    api_keys: ApiKeysConfig = Field(
        default_factory=ApiKeysConfig, description="API key env-var names"
    )
    storage: StorageConfig = Field(
        default_factory=StorageConfig, description="Storage settings"
    )
    sandbox: SandboxConfig = Field(
        default_factory=SandboxConfig, description="Sandboxing settings"
    )
