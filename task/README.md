...
| [05_phase3_execution.md](./05_phase3_execution.md) | Phase 3: 実験実行（Executor）+ 多言語・コンパイル型言語・依存管理 | §7 |
...
| ファイル | 内容 | セクション |
|---------|------|-----------|
| [17_slurm.md](./17_slurm.md) | SLURM実行パイプライン + 分散SLURMアーキテクチャ | §23 |
...
| バージョン | 変更内容 |
|-----------|---------|
| v13.3 | §23.9–§23.14 分散SLURMアーキテクチャ追加（ログインノード＝オーケストレーター、全GPU処理を計算ノードで実行）。StageNodeConfig / DistributedSlurmConfig 追加。vLLMリモートサーバー管理（persistent/transient）。リモートPPO実行。排他モード（--exclusive）。CLI/Wizard 分散SLURM設定オプション追加。§26.5.10c-slurm SLURMノード設定ウィザードステップ追加 |
| v13.2 | §7.3.2 コンパイル型言語サポート（C++, Rust, Go）追加。§7.3.3 動的依存関係管理と LLM 生成ビルドオプション追加。§23.6 SLURM+コンテナ統合（Singularity / Apptainer / Docker）追加 |
