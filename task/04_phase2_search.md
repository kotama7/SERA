...
class SearchNode:
    ...
    # 失敗知識コンテキスト（ECHO軽量版、§25.4.3 — 実装済み）
...
def debug(self, failed_node: SearchNode, agent_llm: AgentLLM,
    ...
    1a. failure_extractor が有効な場合、失敗パターンを分析し兄弟ノードへ知識注入（§25.4.3）
...
これは ECHO軽量版（§25.4.3 FailureKnowledgeExtractor）で抽出された `FailureSummary` を利用する。
...
■ 有効化条件:
  ...
  - echo.enabled == False の場合、failure_context は空のまま（デフォルトは True）
```
