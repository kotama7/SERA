...
> 本ファイルは TASK.md v13.1 の拡張仕様である。目次は [README.md](./README.md) を参照。
...
| オペレータ別集計 | draft / debug / improve のノード数（SVG 円グラフ — `html_renderer.py` で実装済み） |
...
`--live` オプション指定時、stdlib `http.server` ベースの軽量HTTPサーバーを起動してリアルタイム更新を提供する。

```python
# visualize_cmd.py — _run_live_server()
def _run_live_server(workspace, visualizer, html_path, port):
    """
    1. checkpoints/ ディレクトリをポーリング（5秒間隔）で監視
    2. 新チェックポイント検出時に HTML を再生成
    3. 生成HTMLに <meta http-equiv="refresh" content="10"> を注入
    4. ブラウザが10秒ごとに自動リロード
    """
```

**実装方式**: WebSocket ではなく、stdlib のみで実装。
- ファイル監視: `threading.Thread` による `time.sleep(5)` ポーリング（`watchdog` 不要）
- ブラウザ更新: `<meta http-equiv="refresh">` タグ注入（WebSocket 不要）
- HTTP サーバー: `http.server.HTTPServer` + `SimpleHTTPRequestHandler`

**依存パッケージ**: なし（stdlib のみ）。
...
| **P0（必須）** | ノードのステータス別色分け表示 | §29.4.2 配色テーブル |
| **P0（必須）** | ノードクリック → 詳細パネル表示 | §29.4.3 |
| **P0（必須）** | 統計サマリー（ノード数、ステータス集計、ベストノード） | §29.4.4 |
| **P0（必須）** | CLI `sera visualize` コマンド | §29.2 |
| **P1（推奨）** | オペレータ別ノード形状 | draft=丸、debug=三角、improve=四角 |
| **P1（推奨）** | パン・ズーム操作 | D3.js zoom behavior |
| **P1（推奨）** | 実験コード / stdout / stderr モーダル表示 | §29.4.3 下部ボタン |
| **P1（推奨）** | 性能推移グラフ（LCB折れ線） | §29.4.4 |
| **P1（推奨）** | 大規模ツリー対策（フィルタリング） | §29.9 |
| **P2（拡張）** ✅ | ライブモード（meta refresh + ポーリング実装済み） | §29.7 |
...
| パッケージ | 用途 | 追加先 |
|-----------|------|--------|
| なし | D3.js はHTMLインライン埋め込み、ライブモードも stdlib のみで実装 | — |

**依存パッケージなし**。ライブモードも `http.server` + `threading` + `<meta refresh>` で stdlib のみで実装済み。`watchdog`/`aiohttp` は不要。
