...
## 28. Tool Execution Engine（ツール実行基盤）

### 28.1 概要

§27（Agent Function System）で定義された `AgentFunction` のうち、`allowed_tools` を持つ10関数は AgentLoop 経由で実行される。本セクションでは、その実行基盤となるツール実装・AgentLoop メカニズム・安全制御を定義する。
...
■ §27 と §28 の役割分担
  ...
  §27 AgentFunction（タスク定義）
    ├─ 何を生成するか（return_schema, handler）
    ├─ どのツールを使えるか（allowed_tools）
    └─ ループ設定（loop_config: max_steps, budget）

  §28 Tool Execution Engine（実行基盤）
    ├─ ツールの実関数（ToolExecutor + 18 ハンドラ）
    ├─ ReAct ループ制御（AgentLoop）
    └─ 安全制御（ToolPolicy）
```

§27 の `call_function()` が `allowed_tools` の有無と `ToolConfig.enabled` に基づいて本セクションの AgentLoop を起動する。詳細は §27.6 を参照。
...
### 28.3 ToolExecutor（ツール実行ディスパッチャ）
...
class ToolResult:
    ...
    # ストリーミング実行フィールド（§7.5 連携）
    stdout_preview: str | None = None   # 実行ツールの stdout 末尾（最大 20 行）
    stderr_preview: str | None = None   # 実行ツールの stderr 末尾（最大 10 行）
    is_execution: bool = False          # 実行ツールかどうかのフラグ
...
async def execute(self, tool_call: ToolCall) -> ToolResult:
    ...
    try:
        ...
        # 実行ツールの場合、stdout/stderr preview を抽出（§7.5 連携）
...
# 実験実行ツールの例（§7.5 ストリーミング対応）
async def _handle_execute_experiment(self, args: dict) -> dict:
    node_id = args["node_id"]
    script_path = self._workspace / "runs" / node_id / "experiment.py"
    seed = args.get("seed", 42)
    # run_stream() でストリーミングイベントを収集
    stdout_lines, stderr_lines, final_event = [], [], None
    async for event in self._executor.run_stream(node_id, script_path, seed, timeout=self._timeout):
    ...
    return {
        ...
        "metrics": self._read_metrics_safe(final_event.metadata.get("metrics_path")),
        "stderr_tail": "\n".join(stderr_lines[-20:]),
        "stdout_tail": "\n".join(stdout_lines[-20:]),  # 新規
        "stdout_lines": len(stdout_lines),              # 新規
        "stderr_lines": len(stderr_lines),              # 新規
    }
```

> **ストリーミング化要件**（`execution_tools.py` の 3 ハンドラ共通）:
>
> | ID | 要件 |
> |----|------|
> | S-6.1 | `handle_execute_experiment` は `executor.run_stream()` を使用し、ストリーミングイベントを収集 |
> | S-6.2 | 返却 dict に `stdout_tail`, `stderr_tail`, `stdout_lines`, `stderr_lines` を追加 |
> | S-6.3 | `handle_execute_code_snippet` と `handle_run_shell_command` は `asyncio.Queue` ベースの行単位読み取りに変更 |
> | S-6.4 | 出力文字数上限を 5000 → 8000 に引き上げ |
> | S-6.5 | 返却 dict に `stdout_line_count`, `stderr_line_count` を追加 |
> | S-6.6 | タイムアウト処理・エラーハンドリングは既存と同等 |

### 28.4 AgentLoop（エージェント反復ループ）
...
class AgentLoopConfig:
    ...
    on_tool_output: Callable[[ToolResult], None] | None = None  # ツール実行後コールバック（§7.5 連携）
...
**実行ツールのリッチ Observation**（§7.5 連携）: `ToolResult.is_execution == True` の場合、`_format_observations()` は stdout/stderr のインラインプレビューを追加する:

```text
[1] execute_code_snippet (call_id: abc12345)
  Status: success
  Output:
...
| ID | 要件 |
|----|------|
| S-8.1 | `AgentLoopConfig.on_tool_output` は `Callable[[ToolResult], None] | None` 型、デフォルト `None` |
| S-8.2 | `_format_observations()` で `tr.is_execution and tr.stdout_preview` の場合、`--- stdout (last 20 lines) ---` ヘッダ付きでインライン表示 |
| S-8.3 | `_format_observations()` で `tr.is_execution and tr.stderr_preview` の場合、`--- stderr (last 10 lines) ---` ヘッダ付きでインライン表示 |
| S-8.4 | `run()` ループ内で各ツール実行後に `on_tool_output` コールバックを呼び出す（例外は握りつぶして `logger.debug`） |
...
`ToolConfig.enabled = True` がデフォルトであり、全19関数がデフォルトで実行可能となる。ツール実行を無効化するには明示的に `tools.enabled: false` を設定する。

### 28.9 MCP（Model Context Protocol）対応 — ✅ 部分実装

MCP サーバー経由で外部ツールを動的に追加する。`research_cmd.py` にて `ResourceSpec.mcp.servers` を `ToolExecutor` に接続する初期化コードが実装済み。`MCPToolProvider` 本体はスタブ。
...
```python
# src/sera/agent/mcp_client.py（スタブ実装済み、本体は将来実装）
...
#### 28.9.3 ResourceSpec への MCP 設定

```python
# resource_spec.py に追加（✅ 実装済み）
...
Step 5.5: ストリーミング実行（§7.5 連携） ✅ 部分実装
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ✅ 実装済み:
    - src/sera/execution/streaming.py（StreamEvent, StreamEventType — core 5 + extension 2）
    - src/sera/execution/__init__.py（StreamEvent, StreamEventType エクスポート追加）
    - src/sera/agent/agent_loop.py（AgentLoopConfig に on_tool_output コールバック追加、run() でコールバック呼び出し）
  🔲 未着手:
    - src/sera/agent/tools/execution_tools.py（3 ハンドラをストリーミング対応に書き換え）
    - src/sera/agent/tool_executor.py（ToolResult に stdout_preview/stderr_preview/is_execution 追加）
  テスト:
    - tests/test_execution/test_streaming.py（新規: StreamEvent 型、LocalExecutor.run_stream()、基底 run_stream()）
  後方互換性:
    - 既存 run() は変更なし、SlurmExecutor/DockerExecutor はデフォルト run_stream() を自動継承
    - ToolResult の 3 新フィールドは全てデフォルト値あり
    - AgentLoopConfig の on_tool_output はデフォルト None
    - 既存テスト全て変更なしで通過

Step 6: MCP 対応 ✅ 部分実装（research_cmd.py への統合済み）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ✅ 実装済み:
    - src/sera/commands/research_cmd.py にて ResourceSpec.mcp.servers を ToolExecutor に接続するコード追加
      → MCPToolProvider を初期化し tool_executor.add_mcp_provider() で登録
    - src/sera/specs/resource_spec.py に MCPConfig, MCPServerConfig モデル定義済み
  🔲 未着手:
    - src/sera/agent/mcp_client.py（MCPToolProvider 本体 — スタブのみ）
  テスト:
    - tests/test_agent/test_mcp_client.py（🔲 未着手）

Step 7: ツール使用経験の学習統合 🔲 未着手 → §25.5.3 参照
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  前提: Step 1–5 完了 ✅ + Step 5.5（ストリーミング実行）完了
  依存: §7.5（streaming.py）、§25.5.3（PPORolloutV3 + グルーコード）
  詳細は task/19_tool_using_agent.md §25.5.3 を参照

  ■ Step 5.5 → Step 7 の依存関係:
    Step 5.5 で追加される on_tool_output コールバック（AgentLoopConfig）は、
    Step 7b のログ確率キャプチャで使用する。ストリーミングイベントに
    ToolResult の stdout_preview/stderr_preview が含まれるため、
    ツール使用品質の報酬計算（Step 7c）でリッチなフィードバックが得られる。

  ■ 現在の切断箇所（Step 7 で接続すべきもの — 5→3に削減）:
    1. AgentLLM._last_loop_result → SearchManager が未読（7b で接続）
    2. PPORolloutV3 が定義済みだがインスタンス化されない（7a+7b で接続）
    3. log_prob が 0.0 ハードコード（7b で接続）
    ✅ 解決済み: PPOTrainer に tool_aware ルーティング実装（ppo_trainer.py）
    ✅ 解決済み: AgentLoopConfig に on_tool_output コールバック実装（agent_loop.py）
```
...
| セクション | 役割 | 本セクションとの関係 |
|-----------|------|-------------------|
| §25 (Tool-Using Agent) | 信用割当 + ツール使用経験からの学習（§25.5.3） | AgentLoop 軌跡が §25 の学習パイプラインに流入 |
| §27 (Agent Function System) | タスク定義（スキーマ + allowed_tools + loop_config） | §27.6 `call_function()` が §28 の AgentLoop を起動 |
| §28 (本セクション) | ツール実行基盤（ToolExecutor + AgentLoop + ToolPolicy） | — |

```text
§27 call_function()                  §28 AgentLoop
  │                                    │
  ...
  │                               AgentLoopResult
  │                                    │
  │  handler(final_output) ◄───────    │  最終出力を §27 のパーサで処理
  │                                    │
  │                                    ▼
  │                          AgentLLM._last_loop_result に保存
  │                                    │
  │                                    ▼  ← Step 7b で接続（現在は切断）
  │                          SearchManager が _last_loop_result を読み取り
  │                          ToolCallRecord を抽出して PPO バッファに追加
  │                                    │
  │                                    ▼
  │                               §25.5.3 学習統合
...
**現在のデータフロー（Step 7 未実装時）**:
```text
AgentLoop.run()
  │
  ├─ final_output → tree_ops が仮説 JSON を抽出
  │                    │
  │                    ▼
  │               SearchNode 作成 → Executor 実行 → 統計評価
  │                                                    │
  │                                                    ▼
  │                                           PPO バッファ:
  │                                             hypothesis, config,
  │                                             reward=node.mu,
  │                                             turn_rewards=ヒューリスティック,
  │                                             log_prob=0.0 ← ハードコード
  │
  └─ _last_loop_result に保存 → 誰も読まない ← ここが切断箇所
```
...
AgentLoop.run()
  │
  ├─ final_output → tree_ops が仮説 JSON を抽出 → SearchNode
  │
  └─ _last_loop_result ──(Step 7b)──► SearchManager が読み取り
       │                                   │
       ├─ turns[].tool_results[]            ├─ ToolCallRecord[] を構築
       ├─ turns[].generation.log_probs      ├─ tool_log_prob_sum を計算
       └─ total_tool_calls, exit_reason     └─ PPORolloutV3 を生成
                                                  │
                                                  ▼
                                            PPOTrainer.update()
                                              ├─ compute_reward_tool_aware()
                                              └─ HiPER switch-level ツール品質
```

### 28.14 ツール使用経験からの学習統合

> **本セクションの詳細は §25.5.3（task/19_tool_using_agent.md）に移動。** ここでは §28 側のインターフェースのみ定義する。

AgentLoop の実行軌跡を §25 の PPO/LoRA 学習パイプラインにフィードバックするため、以下のインターフェースを提供する:

#### 28.14.1 AgentLoopResult の学習用フィールド

§28.4.2 で定義した `AgentLoopResult` は以下の情報を含み、§25.5.3 の学習統合で使用される:

- `turns: list[AgentTurn]` — 各ステップの生成結果 + ツール実行結果
- `total_steps` / `total_tool_calls` — ループ統計
- `exit_reason` — 終了理由

#### 28.14.2 GenerationOutput のログ確率拡張
...
#### 28.14.3 SearchNode へのツール使用メタデータ
...
データ構造の詳細（PPORolloutV3, ToolCallRecord, tool_aware 報酬, HiPER 拡張等）は §25.5.3 を参照。
...
| `src/sera/agent/mcp_client.py` | 6 | MCPToolProvider（research_cmd.py から呼び出し済み、本体はスタブ） | ✅ 部分実装 |
| `src/sera/execution/streaming.py` | 5.5 | StreamEvent, StreamEventType（core 5 + extension 2） | ✅ 実装済み |
...
| `src/sera/execution/executor.py` | 5.5 | デフォルト `run_stream()` 追加 | 🔲 未着手 |
| `src/sera/execution/local_executor.py` | 5.5 | `run_stream()` オーバーライド | 🔲 未着手 |
| `src/sera/execution/__init__.py` | 5.5 | StreamEvent/StreamEventType エクスポート | ✅ 実装済み |
| `src/sera/agent/tools/execution_tools.py` | 5.5 | 3 ハンドラをストリーミング対応に書き換え | 🔲 未着手 |
| `src/sera/agent/tool_executor.py` | 5.5 | ToolResult 拡張 + preview 設定 | 🔲 未着手 |
| `src/sera/agent/agent_loop.py` | 5.5 | on_tool_output コールバック追加 | ✅ 実装済み |
| `src/sera/specs/resource_spec.py` | 6 | MCPConfig, MCPServerConfig 追加 | ✅ 実装済み |
| `src/sera/commands/research_cmd.py` | 6 | MCP servers → ToolExecutor 接続 | ✅ 実装済み |

> Step 7（学習統合）の修正ファイルは §25.5.3（task/19_tool_using_agent.md）を参照。

---

（§28 Tool Execution Engine — TASK.md v13.1）
