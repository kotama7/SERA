...
ツール呼び出し機能は Phase C（§25）で有効化され、`GenerationOutput` 型で統一的に返される（付録C参照）。
...
10. **ターンレベル信用割当**：各Phaseの出力品質を個別に報酬化し（MT-GRPO、§25.4.2）、フェーズ間の信用割当を明示的に行う。PPO更新時に `R = Σ_t(w_t * r_turn_t) - penalties` で集約される。
11. **失敗知識の再利用**：失敗ノードの知識をECHO軽量版（§25.4.3）で抽出し、兄弟ノードのコンテキストに注入する。失敗は単に破棄するのではなく、後続探索の情報源として活用する。
