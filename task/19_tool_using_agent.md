...
## 25. Tool-Using Agent拡張（HiPER + ECHO統合による信用割当）
...
### 25.4 採用アーキテクチャ：MT-GRPO + ECHO軽量版
...
#### 25.5.3 ツール使用経験からの学習統合

§28（Tool Execution Engine）の AgentLoop でツールを使用した軌跡を、PPO/LoRA 学習パイプラインにフィードバックする。§27 の `call_function()` が AgentLoop を起動した場合、その軌跡が本セクションの仕組みで学習に活用される。
...
完全な引数は §28.7 の `tool_execution_log.jsonl` に記録済み。PPO バッファにはハッシュのみ保持（メモリ効率）。
...
class PPORolloutV3(PPORolloutV2):
    ...
    tool_success_rate: float = 1.0       # ツール成功率（デフォルト 1.0: ツール未使用時は全成功扱い）
...
§27.6 の `call_function()` が AgentLoop を使用した場合、`_last_loop_result` に `AgentLoopResult` が保持される。SearchManager はこれを PPO バッファに統合する:
...
ツール選択のログ確率は AgentLoop 実行時にキャプチャする（§28.14.2）。PPO 更新時の再計算はテキスト部分のみ可能で、ツール実行結果の観測コンテキストが失われるため再現不可能。
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  前提:
    - §28 Step 1–5（ToolExecutor + AgentLoop + Phase統合）完了 ✅
    - §28 Step 5.5（ストリーミング実行、§7.5）完了
      → on_tool_output コールバック、ToolResult.is_execution/stdout_preview/stderr_preview
      → execute_experiment の返却 dict に stdout_tail/stderr_tail/stdout_lines/stderr_lines 追加

  7a: データ構造拡張
    - src/sera/learning/rollout.py（ToolCallRecord, PPORolloutV3）← 定義済み、未使用
    - src/sera/search/search_node.py（tool_usage フィールド）← 定義済み、ToolExecutor が更新中
    - src/sera/agent/agent_llm.py（GenerationOutput ログ確率フィールド）← 定義済み、未ポピュレート

  7b: グルーコード（AgentLoop → PPO バッファ接続）★ 最重要
    - src/sera/search/search_manager.py:
      (1) tree_ops の call_function() 後に agent_llm._last_loop_result を読み取る
      (2) AgentLoopResult.turns から ToolCallRecord[] を構築する
      (3) PPO バッファエントリに tool_trajectory, tool_log_prob_sum 等を追加
      (4) PPORolloutV2 ではなく PPORolloutV3 をインスタンス化
    - src/sera/agent/agent_loop.py:
      (1) 各ステップで GenerationOutput.text_log_prob / tool_call_log_probs を記録
      (2) Step 5.5 の on_tool_output コールバックと連携

  7c: 報酬関数拡張
    - src/sera/learning/reward.py（compute_reward_tool_aware）← 登録済み、データ未到達
    - src/sera/specs/plan_spec.py（ToolRewardConfig）
    - SearchManager が compute_reward() 呼び出し時に tool_* kwargs を渡すよう修正
...
Step 7b は**最重要**のステップであり、現在切断されているデータフローを接続する。

**SearchManager での _last_loop_result 読み取り**:

```python
# search_manager.py — PPO バッファ構築時（_evaluate_node 内）
# tree_ops.draft/debug/improve は内部で agent_llm.call_function() を呼ぶ。
# call_function() が AgentLoop を使用した場合、agent_llm._last_loop_result に
# AgentLoopResult が保持される。SearchManager はこれを PPO バッファに統合する。

def _build_tool_trajectory(self, loop_result: AgentLoopResult) -> list[ToolCallRecord]:
    """AgentLoopResult から ToolCallRecord リストを構築する。"""
    records = []
    for turn in loop_result.turns:
        tool_call_lps = turn.generation.tool_call_log_probs or []
        for i, tr in enumerate(turn.tool_results):
            ...
            records.append(ToolCallRecord(
                ...
                arguments_hash=hashlib.sha256(
                    json.dumps(tr.output, default=str, sort_keys=True).encode()
                ).hexdigest()[:16],
                success=tr.success,
...
# PPO バッファ追加時:
entry = {"node_id": ..., "hypothesis": ..., "config": ..., "reward": ..., ...}
...
if loop_result is not None:
    ...
    entry["tool_trajectory"] = [t.__dict__ for t in tool_trajectory]
    entry["tool_log_prob_sum"] = sum(t.log_prob for t in tool_trajectory)
    entry["text_log_prob_sum"] = sum(
        turn.generation.text_log_prob or 0.0 for turn in loop_result.turns
    )
    entry["total_tool_calls"] = loop_result.total_tool_calls
    entry["tool_success_rate"] = (
        sum(1 for t in tool_trajectory if t.success) / len(tool_trajectory)
        if tool_trajectory else 1.0
    )
    entry["agent_loop_steps"] = loop_result.total_steps
    entry["agent_loop_exit_reason"] = loop_result.exit_reason
    # _last_loop_result をクリア（次のノードと混同しないため）
    self.agent_llm._last_loop_result = None

# PPO 更新時のロールアウト生成:
if "tool_trajectory" in entry:
    rollouts.append(PPORolloutV3(
        node_id=entry["node_id"],
        prompt=entry.get("hypothesis", ""),
        response=str(entry.get("config", {})),
        log_prob=entry.get("text_log_prob_sum", 0.0) + entry.get("tool_log_prob_sum", 0.0),
        reward=entry.get("reward", 0.0),
        value=0.0,
        turn_rewards=entry.get("turn_rewards", {}),
        tool_trajectory=[ToolCallRecord(**t) for t in entry["tool_trajectory"]],
        tool_log_prob_sum=entry.get("tool_log_prob_sum", 0.0),
        text_log_prob_sum=entry.get("text_log_prob_sum", 0.0),
        total_tool_calls=entry.get("total_tool_calls", 0),
        tool_success_rate=entry.get("tool_success_rate", 1.0),
        agent_loop_steps=entry.get("agent_loop_steps", 0),
        agent_loop_exit_reason=entry.get("agent_loop_exit_reason", ""),
    ))
else:
    # ツール未使用時は従来通り PPORolloutV2 or PPORollout
    ...
```

| ID | 要件 |
|----|------|
| L-7b.1 | SearchManager は各 tree_ops 操作後に `agent_llm._last_loop_result` を確認する |
| L-7b.2 | `_last_loop_result` が存在する場合、`_build_tool_trajectory()` で ToolCallRecord[] を構築 |
| L-7b.3 | PPO バッファエントリに `tool_trajectory`, `tool_log_prob_sum`, `text_log_prob_sum`, `total_tool_calls`, `tool_success_rate`, `agent_loop_steps`, `agent_loop_exit_reason` を追加 |
| L-7b.4 | 読み取り後に `_last_loop_result = None` でクリア（ノード間の混同防止） |
| L-7b.5 | PPO 更新時、`tool_trajectory` が存在するエントリは `PPORolloutV3` としてインスタンス化 |
| L-7b.6 | ツール未使用ノードは従来通り `PPORolloutV2` or `PPORollout`（後方互換） |

### 25.5.4 アーキテクチャ一貫性検証（AgentLLM + LoRA + BFS + HiPER + ECHO）

> **検証日**: 2026-02-25

以下は、SERA の主要5コンポーネントが矛盾なく一貫して統合されていることの検証結果である。

**統合検証マトリクス**：

| 統合パス | 接続点 | 状態 | 検証 |
|---------|--------|------|------|
| AgentLLM → LoRA | `load_adapter()` → `lineage_manager.materialize()` → `peft.set_peft_model_state_dict()` | ✅ | research_cmd.py:196 |
| LoRA → LineageManager | `save_delta()` / `materialize()` / delta inheritance | ✅ | lineage_manager.py |
| BFS → AgentLLM | `tree_ops.draft/debug/improve()` → `agent_llm.generate()` / `call_function()` with `adapter_node_id` | ✅ | tree_ops.py |
| BFS → PPOTrainer | `ppo_buffer.append(entry)` → `ppo_trainer.update(rollouts)` | ✅ | search_manager.py:245-293 |
| PPOTrainer → LoRA | `update()` → delta computation → `lineage_manager.save_delta()` | ✅ | ppo_trainer.py |
| PPOTrainer → HiPER | `method=="hiper"` → `HierarchicalAdvantageEstimator` 3層分解 | ✅ | ppo_trainer.py:169-183 |
| HiPER → TurnReward | `turn_rewards_map` → switch_level variance computation | ✅ | hierarchical_ppo.py |
| ECHO → BFS | `failure_extractor.extract()` → `inject(summary, siblings)` | ✅ | search_manager.py:173-187 |
| ECHO → TreeOps | `{failure_context}` in IMPROVE_PROMPT | ✅ | tree_ops.py |
| Adapter Sync → SearchNode | 3-rule: root→adapter_root, ppo→new_id, inherit→parent | ✅ | search_manager.py:25-49 |
| AgentLoop → ToolExecutor | `agent_loop.run()` → `tool_executor.execute()` → ToolPolicy | ✅ | agent_loop.py |
| vLLM sleep/wake | PPO前にsleep(2)、PPO後にwake → GPU排他 | ✅ | research_cmd.py |

**デフォルト設定での動作フロー**（provider=local, learning.enabled=true, reward.method=hiper, echo.enabled=true）:

```text
Phase 1: Specs frozen (agent_commands + tools.enabled=true + echo + hiper)
    ↓
Phase 2: SearchManager.run()
    ├── draft(5 roots) → AgentLLM.call_function("search_draft", adapter_node_id="adapter_root")
    │     └── AgentLoop: get_node_info, list_nodes, read_metrics → hypothesis JSON
    ├── evaluate → Executor.run() → metrics.json → node.mu, node.se, node.lcb
    │     ├── TurnRewardEvaluator → {phase0: 0.2, phase2: 0.5, phase3: 0.8, ...}
    │     └── ppo_buffer.append({reward, turn_rewards})
    ├── select_next(improve) → AgentLLM.call_function("search_improve", adapter_node_id=parent.adapter_id)
    ├── [failed node] → ECHO: extract(node) → inject(summary, siblings)
    │     └── debug → AgentLLM.call_function("search_debug") with failure_context
    ↓
Phase 5: PPO trigger (every 5 evaluated nodes)
    ├── vLLM.sleep(level=2) → free GPU
    ├── rollouts = [PPORolloutV2(reward, turn_rewards)]
    ├── HiPER: high_adv(0.4) + switch_adv(0.3, phase_variance) + low_adv(0.3, mean_phase)
    ├── LoRA-only update → new delta
    ├── lineage_manager.save_delta(new_adapter_id, parent_id, delta)
    ├── sync_adapter_assignment → nodes get new adapter_node_id
    └── vLLM.wake() → resume inference with new adapter
```

**未解決の断絶点（Step 7b）**: PPORolloutV3 によるツール使用軌跡の PPO 統合のみ。これは LoRA/BFS/HiPER/ECHO の中核パスには影響しない。ツール使用報酬は将来の拡張であり、現在のデフォルト設定（`reward.method=hiper`）で HiPER 3層分解は正常動作する。

### 25.6 段階的実装ロードマップ

> **実装状況更新**: Phase A + Phase B + Phase C の報酬/Advantage部分およびAgent Function System / Tool Execution Engineは**全て実装済み**。設定ファイル（`plan_spec.yaml`）から全手法を切り替え可能。残りは §25.5.3 のツール使用経験からの学習統合（PPORolloutV3等）と MCP 対応のみ。
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  §27 Agent Function System（task/21_agent_functions.md）: ✅ 全項目実装済み
    ✅ 実装済み:
  ...
  §28 Tool Execution Engine（task/22_tool_execution.md）: ✅ 全項目実装済み（MCP除く）
    ✅ 実装済み:
  ...
  §28 Step 5.5 ストリーミング実行（task/22_tool_execution.md）:
    🔲 未着手:
      - streaming.py（StreamEvent, StreamEventType）
      - executor.py / local_executor.py（run_stream）
      - execution_tools.py（3 ハンドラのストリーミング化）
      - tool_executor.py（ToolResult 拡張: stdout_preview/stderr_preview/is_execution）
      - agent_loop.py（on_tool_output コールバック + リッチ観測）
    → Step 7 の前提。特に on_tool_output と ToolResult.is_execution は 7b で活用。

  §25.5.3 ツール使用経験からの学習統合（本ファイル上記）:
    ✅ 部分実装:
      - PPOTrainer に `tool_aware` reward method ルーティング実装済み（ppo_trainer.py）
        → PPORolloutV3.tool_trajectory から ToolCallRecord をパースし compute_reward_tool_aware を呼び出す
      - AgentLoopConfig に `on_tool_output` コールバック実装済み（agent_loop.py）
        → ストリーミング実行時のツール出力をリアルタイム通知可能
    🔲 未着手:
      - グルーコード: SearchManager ↔ _last_loop_result 接続（Step 7b）★最重要
      - HiPER switch-level ツール品質（Step 7d）

    ■ 現在の切断箇所（3箇所、5→3に削減）:
      1. AgentLLM._last_loop_result が保存されるが SearchManager が読まない
      2. PPORolloutV3 が定義済みだがインスタンス化されない
      3. log_prob が 0.0 ハードコード
    → Step 7b のグルーコード実装で解消される。詳細は上記 §25.5.3 参照。

  実装順序: Step 5.5 → Step 7a → Step 7b → Step 7c → Step 7d
  前提条件: HiPER/ECHOの有効性がアブレーションで確認済み
  検証: 全アブレーション再実行 + tool-calling特有の評価指標追加
```

### 25.7 ECHO-2（分散RL基盤）との関連
...
| `src/sera/agent/agent_functions.py` | C | ✅ 実装済み | AgentFunction / AgentFunctionRegistry / REGISTRY (§27) |
| `src/sera/agent/functions/*.py` | C | ✅ 実装済み | 全19関数の定義・ハンドラ (§27) |
| `src/sera/agent/tool_executor.py` | C | ✅ 実装済み | ToolResult / ToolExecutor / ディスパッチ (§28) |
| `src/sera/agent/tool_policy.py` | C | ✅ 実装済み | ToolPolicy / パス制約 / レート制限 (§28) |
| `src/sera/agent/tools/*.py` | C | ✅ 実装済み | 18個のツールハンドラ (§28) |
| `src/sera/agent/agent_loop.py` | C | ✅ 実装済み | AgentLoop — ReAct型反復ループ (§28) |
| `src/sera/agent/mcp_client.py` | C | 🔲 未着手 | MCP対応（将来） (§28) |
...
**§25.5.3 学習統合で追加される修正** (Phase C — Step 7):
...
（このTASK.mdは完全最終版 v13.1（全面Agent対応更新版）である。v13.1 での変更：§27 Agent Function System / §28 Tool Execution Engine の全項目を実装済みに更新、§25.5.3 に PlanSpec §5.8 agent_commands 参照を追加、§25.2 B5 を解決済みに更新、§25.6 Phase C ロードマップを実装状況に合わせて改訂、§25.8 ファイル一覧の状態フラグを更新。v12.4 からの変更：§0-§22を tool-calling + MT-GRPO + ECHO軽量版前提に改訂。§0.1 エージェント定義をtool-calling対応に書き換え、§1 設計原則に信用割当原則追加、§2.1 データフロー図にターン報酬・失敗知識注入追加、§6 にECHO失敗知識抽出統合、§9 にPPORolloutV2/compute_reward_v2/TurnRewardSpec追加、§11-12 をエージェント的反復改善/マルチエージェントレビューとして再フレーミング、§19-20 受け入れ基準・MVP・モジュール構成にAgent機能追加、付録C AgentLLMインターフェースをtool-calling対応に全面書き換え、§22 実装手順書にStep 13-15追加。v12.2 からの追加：§24 Agentモデル拡張、§25 Tool-Using Agent拡張。v12.1 からの追加：§23 SLURM実行パイプライン。v12 からの追加：§23 多言語実験サポート）
